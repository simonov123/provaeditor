...
public class runtime {
	...
	private String csvpath="C:\\Users\\Public\\Documents";
	...
	public String setlang(Strings ss) throws IOException {
		...
		File langfile=new File("C:\\Users\\Public\\Documents\\lang.txt");
		if(langfile.exists()==true)
		{
			Scanner scanner=new Scanner(langfile,"UTF-8");
		...
		if(langfile.exists()==false) {
			...
			langbox.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent acb) {
					 ...
					 selang=selectedlang;
					 try {
						BufferedWriter lff = Files.newBufferedWriter(langfile.toPath(), StandardCharsets.UTF_8);
	...
	public void infosaver(String pat, String filename2, String name, int size) throws IOException, CsvValidationException {
		File update=new File(csvpath+"\\"+filename2+".csv");
		CSVWriter wr2=new CSVWriter(new FileWriter(csvpath+"\\"+filename2+".csv"));
		String[] filer= {""};
		Integer siz=size;
	...
	public Font getfontinfo(String filename2, String pat) throws CsvValidationException, IOException {
		File getinfo=new File(csvpath+"\\"+filename2+".csv");
		if (getinfo.exists()==true)
		{
		CSVReader r1=new CSVReader(new FileReader(csvpath+"\\"+filename2+".csv"));
